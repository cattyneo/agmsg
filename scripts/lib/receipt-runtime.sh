...
_agmsg_receipt_runtime_cleanup() {
  ...
  if [ -n "${_AGMSG_RECEIPT_RUNTIME_TMP:-}" ] &&
    ...
    /bin/rm -rf -- "$_AGMSG_RECEIPT_RUNTIME_TMP"
  fi
  _AGMSG_RECEIPT_RUNTIME_TMP=
}
...
_agmsg_receipt_runtime_run_scoped() (
  ...
  _AGMSG_RECEIPT_RUNTIME_TMP="$(/usr/bin/mktemp -d \
    "${TMPDIR:-/tmp}/agmsg-receipt-runtime.XXXXXX")" || {
    printf '%s\n' 'agmsg receipt: cannot create private temporary directory' >&2
    exit 1
  }
  /bin/chmod 700 "$_AGMSG_RECEIPT_RUNTIME_TMP" || {
    _agmsg_receipt_runtime_cleanup
    printf '%s\n' 'agmsg receipt: cannot protect temporary directory' >&2
    exit 1
  }
  trap _agmsg_receipt_runtime_cleanup EXIT
...
_agmsg_receipt_runtime_openssl_probe() {
  ...
  IFS=' ' read -r digest _ <"$output" || true
  ...
  IFS=' ' read -r _AGMSG_RECEIPT_PUBLIC_KEY_SHA256 _ <"$output" || true
...
_agmsg_receipt_runtime_xxd_probe() {
  local xxd="$1" openssl="$2" i=0 digest
  ...
  if ! _agmsg_receipt_runtime_run "$xxd" -r -p "$canonical" "$original" \
     ...
     ! _agmsg_receipt_runtime_run "$xxd" -r -p "$encoded" "$restored" \
      >"$output" 2>&1 ||
     [ "$(/usr/bin/wc -c <"$original" | /usr/bin/tr -d ' ')" != 258 ] ||
     ! /usr/bin/cmp -s "$original" "$restored"; then
  ...
  if _agmsg_receipt_runtime_run "$xxd" -v >"$output" 2>&1; then
    IFS= read -r _AGMSG_RECEIPT_XXD_VERSION <"$output" || true
...
_agmsg_receipt_runtime_select() {
  local os="$1" tool="$2" override="$3" candidate candidates error_file
  error_file="$_AGMSG_RECEIPT_RUNTIME_TMP/$tool.error"
  _AGMSG_RECEIPT_RUNTIME_SELECTED=
  ...
  if [ -n "$override" ]; then
    ...
    if [ "$tool" = openssl ]; then
      _agmsg_receipt_runtime_openssl_probe "$override" 2>"$error_file" || {
        /bin/cat "$error_file" >&2
        return 1
      }
    else
      _agmsg_receipt_runtime_xxd_probe "$override" \
        "$AGMSG_RECEIPT_OPENSSL_RESOLVED" 2>"$error_file" || {
        /bin/cat "$error_file" >&2
        return 1
      }
    fi
  ...
  while IFS= read -r candidate; do
    ...
    if [ "$tool" = openssl ]; then
      if _agmsg_receipt_runtime_openssl_probe "$candidate" \
          >"$error_file" 2>&1; then
        _AGMSG_RECEIPT_RUNTIME_SELECTED="$candidate"
        return 0
      fi
    elif _agmsg_receipt_runtime_xxd_probe "$candidate" \
        "$AGMSG_RECEIPT_OPENSSL_RESOLVED" >"$error_file" 2>&1; then
      _AGMSG_RECEIPT_RUNTIME_SELECTED="$candidate"
      return 0
    fi
  done <<EOF
...
_agmsg_receipt_runtime_resolve_body() {
  ...
  _agmsg_receipt_runtime_select "$os" openssl \
    "${AGMSG_RECEIPT_OPENSSL:-}" || return 1
...
agmsg_receipt_resolve_runtime() {
  ...
  {
    IFS= read -r openssl_path || return 1
    IFS= read -r xxd_path || return 1
    IFS= read -r openssl_version || return 1
    IFS= read -r xxd_version || return 1
    IFS= read -r fixture_sha || return 1
    IFS= read -r public_sha || return 1
    IFS= read -r extra || true
  } <<EOF
$result
EOF
  [ -z "$extra" ] || return 1
  case "$openssl_path:$xxd_path" in /*:/*) ;; *) return 1 ;; esac
  _agmsg_receipt_runtime_is_sha256 "$fixture_sha" || return 1
  _agmsg_receipt_runtime_is_sha256 "$public_sha" || return 1
